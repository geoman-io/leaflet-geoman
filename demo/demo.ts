...
map2.on('pm:create', (e) => {
  // (e.layer as L.Polyline).pm.enable({ allowSelfIntersection: false });
  // (e.layer as L.Polyline).pm.disable();
  // console.log((e.layer as L.Polyline).pm.hasSelfIntersection());
...
map2.on('pm:drawstart', (e) => {
  ...
  layer.on('pm:vertexadded', (x) => {
    console.log(x);
    console.log((x.workingLayer as L.Polyline).pm.hasSelfIntersection());
  });
});
