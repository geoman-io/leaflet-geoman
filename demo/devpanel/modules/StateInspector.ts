...
class StateInspector {
  ...
  declare _updateInterval: ReturnType<typeof setInterval> | null;
  ...
  _renderGlobalOptions() {
    const options = this.map.pm.getGlobalOptions() as Record<string, unknown>;
    const defaultOptions: Record<string, boolean | number> = {
      snappable: true,
      snapDistance: 20,
      snapMiddle: false,
      snapSegment: true,
  ...
  _renderToolbarState() {
    const toolbar = this.map.pm.Toolbar as typeof this.map.pm.Toolbar & {
    ...
    if (isVisible && toolbar._toolbarContainer) {
      const buttons =
        toolbar._toolbarContainer.querySelectorAll<HTMLElement>(
          '.button-container'
        );
      const activeButtons = Array.from(buttons)
        .filter((btn) => btn.classList.contains('active'))
        .map((btn) => {
          const buttonEl = btn.querySelector<HTMLElement>(
    ...
    this.container
      ...
      .forEach((input) => {
        input.addEventListener('change', (e) => {
          const option = (e.target as HTMLInputElement).dataset.option!;
          let value;
  ...
  _updateState() {
    // Update modes
    const modeIndicator =
      this.container.querySelector<HTMLElement>('#mode-indicator')!;
    if (modeIndicator) {
    ...
    // Update active shape
    const activeShapeEl =
      this.container.querySelector<HTMLElement>('#active-shape')!;
    if (activeShapeEl) {
    ...
    const toolbarContent = this.container.querySelector<HTMLElement>(
  ...
  _syncGlobalOptionsUI() {
    ...
    this.container
      .querySelectorAll<HTMLElement>('.devpanel-toggle-switch')
      .forEach((row) => {
        const input = row.querySelector<HTMLInputElement>('[data-option]')!;
        if (!input) return;
        ...
        let badge = row.querySelector<HTMLElement>('.devpanel-badge.warning')!;
        if (!isDefault && !badge) {
...
declare global {
  interface Window {
    StateInspector: typeof StateInspector;
  }
}
