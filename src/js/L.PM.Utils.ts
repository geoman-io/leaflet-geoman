...
const Utils: IUtils = {
  calcMiddleLatLng(map, latlng1, latlng2) {
    ...
    return map.unproject(p1.add(p2).divideBy(2));
  },
  findLayers(map) {
    let layers: PMLayer[] = [];
    map.eachLayer((layer) => {
  ...
  _fireEvent(layer, type, data, propagate = false) {
    ...
    groups.forEach((group) => {
      (group as unknown as PMLayer).fire(type, data, propagate);
    });
  },
  ...
  _getIndexFromSegment(coords, segment) {
    if (segment && segment.length === 2) {
      ...
      let newIndex = Math.max(indexA.index ?? 0, indexB.index ?? 0);
  ...
  _getRotatedRectangle(A, B, rotation, map) {
    ...
    const p1 = _toLatLng(map, L.point(x0, y0));
    const p2 = _toLatLng(map, endPoint);
    const p3 = _toLatLng(map, L.point(x1, y1));
    return [p0, p1, p2, p3];
  },
