...
export interface RotateMixinContext {
  ...
  _fireRotationStart: (
    layer: L.Layer | L.Map,
    originLatLngs: L.LatLng[] | L.LatLng[][]
  ) => void;
  _fireRotation: (
    layer: L.Layer | L.Map,
    angleDiff: number,
...
const RotateMixin: IRotateMixin & ThisType<RotateMixinContext & IRotateMixin> =
  {
    _onRotateStart(e) {
      ...
      this._initialRotateLatLng = copyLatLngs(
        this._layer as unknown as L.Polygon
      );
      ...
      this._fireRotationStart(
        this._rotationLayer!,
        originLatLngs as L.LatLng[] | L.LatLng[][]
      );
      this._fireRotationStart(
        this._map,
        originLatLngs as L.LatLng[] | L.LatLng[][]
    ...
    _onRotate(e) {
      ...
      function forEachLatLng(
        ...
        if (L.Util.isArray(latlng[0])) {
          (latlng as (L.LatLng[] | L.LatLng[][])[]).forEach((x, i) =>
            forEachLatLng(
              x as L.LatLng[] | L.LatLng[][] | L.LatLng[][][],
              path.slice(),
              i
            )
          );
          ...
          const markers =
            path.length > 0
              ? get(that._markers, path)
              : (that._markers as L.Marker[][])[0];
      ...
      this._rotationLayer!.setLatLngs(
        this._rotateLayer(
          angleDiffRadiant,
          this._rotationLayer!.pm._rotateOrgLatLng as
            | L.LatLng[]
            | L.LatLng[][]
            | L.LatLng[][][],
          this._rotationOriginLatLng!,
    ...
    _getRotationCenter() {
      ...
      const polygon = L.polygon(
        this._layer.getLatLngs() as L.LatLng[] | L.LatLng[][],
        {
          stroke: false,
          fill: false,
          pmIgnore: true,
    ...
    enableRotate() {
      ...
      if (this._layer instanceof L.Rectangle && this._angle === undefined) {
        ...
        this.setInitAngle(
          calcAngle(this._layer._map, latlngs[0][0], latlngs[0][1]) || 0
        );
      }
      ...
      this._rotatePoly.pm.setOptions(
        (this._layer._map as L.Map & { pm: PMMap }).pm.getGlobalOptions()
      );
      this._rotatePoly.pm.setOptions({
        rotate: true,
        snappable: false,
      ...
      this._rotateOrgLatLng = copyLatLngs(
        this._layer as unknown as L.Polygon
      ) as L.LatLng[] | L.LatLng[][];
    ...
    rotateLayer(degrees) {
      ...
      this._rotateOrgLatLng = L.polygon(
        this._layer.getLatLngs() as L.LatLng[] | L.LatLng[][]
      ).getLatLngs() as L.LatLng[] | L.LatLng[][];
