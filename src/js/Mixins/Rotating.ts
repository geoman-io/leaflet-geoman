...
import { _convertLatLngs, _toPoint, LatLngOutput } from '../helpers/ModeHelper';
import { calcAngle, copyLatLngs } from '../helpers';
import { MatrixInstance } from '../helpers/Matrix';
...
interface RotationLayerPM {
  _rotateOrgLatLng?: L.LatLng[] | L.LatLng[][];
  _layerRotated?: boolean;
  _setAngle: (angle: number) => void;
  _fireEdit: (layer: L.Layer, source: string) => void;
  _fireChange: (coords: L.LatLng[] | L.LatLng[][] | L.LatLng[][][], source: string) => void;
...
export interface RotateMixinContext {
  ...
  _fireUpdate: () => void;
  _fireRotationStart: (layer: L.Layer | L.Map, originLatLngs: L.LatLng[] | L.LatLng[][]) => void;
  _fireRotation: (layer: L.Layer | L.Map, angleDiff: number, oldLatLngs: L.LatLng[] | L.LatLng[][] | L.LatLng[][][], sourceLayer?: L.Layer) => void;
  _fireRotationEnd: (layer: L.Layer | L.Map, startAngle: number, originLatLngs: L.LatLng[] | L.LatLng[][]) => void;
...
const RotateMixin: IRotateMixin & ThisType<RotateMixinContext & IRotateMixin> = {
  _onRotateStart(e) {
    ...
    // Cast to Polygon since copyLatLngs expects Polygon | Polyline
    this._initialRotateLatLng = copyLatLngs(this._layer as unknown as L.Polygon);
    this._startAngle = this.getAngle();

    const originLatLngs = copyLatLngs(
      this._rotationLayer! as unknown as L.Polygon,
      this._rotationLayer!.pm._rotateOrgLatLng
    );

    this._fireRotationStart(this._rotationLayer!, originLatLngs as L.LatLng[] | L.LatLng[][]);
    this._fireRotationStart(this._map, originLatLngs as L.LatLng[] | L.LatLng[][]);
  },
  _onRotate(e) {
    ...
    const that = this;
    function forEachLatLng(latlng: L.LatLng[] | L.LatLng[][] | L.LatLng[][][], path: number[] = [], _i = -1) {
      if (_i > -1) {
        path.push(_i);
      }
      if (L.Util.isArray(latlng[0])) {
        (latlng as (L.LatLng[] | L.LatLng[][])[]).forEach((x, i) => forEachLatLng(x as L.LatLng[] | L.LatLng[][] | L.LatLng[][][], path.slice(), i));
      } else {
        const markers =
          path.length > 0 ? get(that._markers, path) : (that._markers as L.Marker[][])[0];
        (latlng as L.LatLng[]).forEach((_latlng, j) => {
          const marker = (markers as L.Marker[])[j];
    ...
    const oldLatLngs = copyLatLngs(this._rotationLayer! as unknown as L.Polygon);
    // rotate the origin layer
    this._rotationLayer!.setLatLngs(
      this._rotateLayer(
        angleDiffRadiant,
        this._rotationLayer!.pm._rotateOrgLatLng as L.LatLng[] | L.LatLng[][] | L.LatLng[][][],
        this._rotationOriginLatLng!,
        L.PM.Matrix.init(),
        this._map
      ) as L.LatLng[] | L.LatLng[][] | L.LatLng[][][]
  ...
  _onRotateEnd() {
    ...
    this._rotationLayer!.pm._rotateOrgLatLng = copyLatLngs(this._rotationLayer! as unknown as L.Polygon) as L.LatLng[] | L.LatLng[][];

    this._fireRotationEnd(this._rotationLayer!, startAngle, originLatLngs as L.LatLng[] | L.LatLng[][]);
    this._fireRotationEnd(this._map, startAngle, originLatLngs as L.LatLng[] | L.LatLng[][]);
  ...
  _getRotationCenter() {
    ...
    const polygon = L.polygon(this._layer.getLatLngs() as L.LatLng[] | L.LatLng[][], {
      stroke: false,
      fill: false,
      pmIgnore: true,
  ...
  enableRotate() {
    ...
    this._rotatePoly = L.polygon(this._layer.getLatLngs() as L.LatLng[] | L.LatLng[][], options) as unknown as RotatableLayer;
    ...
    this._rotatePoly.pm.setOptions((this._layer._map as L.Map & { pm: PMMap }).pm.getGlobalOptions());
    ...
    this._rotateOrgLatLng = copyLatLngs(this._layer as unknown as L.Polygon) as L.LatLng[] | L.LatLng[][];
  ...
  rotateLayer(degrees) {
    ...
    // store the new latlngs
    this._rotateOrgLatLng = L.polygon(this._layer.getLatLngs() as L.LatLng[] | L.LatLng[][]).getLatLngs() as L.LatLng[] | L.LatLng[][];
