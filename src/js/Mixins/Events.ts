...
/**
 * Event Mixin - provides event firing methods for all PM classes
 */
const EventMixin: IEventMixin & ThisType<EventMixinContext & IEventMixin> = {
  ...
  _fireCenterPlaced(source = 'Draw', customPayload = {}) {
    ...
    this.__fire(
      ...
      'pm:centerplaced',
      {
        ...
        workingLayer,
        layer,
        latlng: (this._layer as L.CircleMarker).getLatLng(),
      },
      source,
      customPayload
    );
  ...
  _fireEdit(fireLayer?, source = 'Edit', customPayload = {}) {
  ...
  _fireRotation(
    ...
    const layer = rotationLayer ?? this._rotationLayer;
    this.__fire(
      fireLayer,
      'pm:rotate',
      {
