...
const EditLine = (
  ...
  _shape: 'Line',
  initialize(this: IEditLine, layer: L.Polyline) {
    this._layer = layer as ExtendedPolyline;
    this._enabled = false;
  },
  enable(this: IEditLine, options?: Partial<LineEditOptions>) {
    ...
    if (!this.options.allowSelfIntersection) {
      this._layer.on(
        'pm:vertexremoved',
        this._handleSelfIntersectionOnVertexRemoval as L.LeafletEventHandlerFn,
        this
      );
    }
  ...
  _initMarkers(this: IEditLine) {
    ...
    const handleRing = (coordsArr: L.LatLng[] | L.LatLng[][]): MarkerArray => {
      // if there is another coords ring, go a level deep and do this again
      if (Array.isArray(coordsArr[0])) {
        return (coordsArr as L.LatLng[][]).map(handleRing, this);
      }
    ...
    // create markers
    this._markers = handleRing(coords as L.LatLng[] | L.LatLng[][]);
    ...
    if (this.options.rotate) {
      ...
      if (!this.options.preventMarkerRemoval) {
        marker.on(
          ...
          this._removeMarker as L.LeafletEventHandlerFn,
          this
        );
      }
    }
  ...
  _createMiddleMarker(
    ...
    middleMarker.on(
      this.options.addVertexOn!,
      this._onMiddleMarkerClick as L.LeafletEventHandlerFn,
      this
    );
    middleMarker.on(
      'movestart',
      this._onMiddleMarkerMoveStart as L.LeafletEventHandlerFn,
      this
    );
  ...
  _onMiddleMarkerMoveStart(
    ...
    middleMarker.on(
      'moveend',
      this._onMiddleMarkerMoveEnd as L.LeafletEventHandlerFn,
      this
    );
    if (!this._vertexValidation('add', e)) {
      middleMarker.on(
        'move',
        this._onMiddleMarkerMovePrevent as L.LeafletEventHandlerFn,
        this
      );
      return;
    }
  ...
  _addMarker(
    ...
    const markerArr: ExtendedMarker[] =
      indexPath.length > 1
        ? (get(this._markers, parentPath) as ExtendedMarker[])
        : (this._markers as ExtendedMarker[]);
  ...
  _removeMarker(
    ...
    // coords of the layer
    let coords = this._layer.getLatLngs() as L.LatLng[] | L.LatLng[][];
  ...
  updatePolygonCoordsFromMarkerDrag(this: IEditLine, marker: ExtendedMarker) {
    ...
    const coords = this._layer.getLatLngs() as L.LatLng[] | L.LatLng[][];

    // get marker latlng
    const latlng = marker.getLatLng() as L.LatLng & { alt?: number };
    ...
    latlng.alt = (parent[index] as L.LatLng & { alt?: number }).alt;
...
(Edit as unknown as { Line: unknown }).Line = EditLine;
