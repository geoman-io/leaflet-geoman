...
interface ToolbarButtonOptions {
  ...
  title: string;
  jsClass?: string;
  onClick: (e?: Event, ctx?: ButtonClickContext) => void;
  afterClick: (e?: Event, ctx?: ButtonClickContext) => void;
  doToggle: boolean;
  toggleStatus: boolean;
  disableOtherButtons: boolean;
  disableByOtherButtons?: boolean;
  position: string;
  tool?: 'edit' | 'options' | 'custom' | string;
 ...
 */
interface IPMButton {
  _button: ToolbarButtonOptions;
  toggle: (status?: boolean) => boolean;
  toggled: () => boolean;
  remove: () => void;
...
const Toolbar = (L.Class as { extend: (props: object) => unknown }).extend({
  options: {
    drawMarker: true,
    drawRectangle: true,
    drawPolyline: true,
  ...
  init(this: IToolbar, map: L.Map) {
    this.map = map as unknown as ExtendedMap;
  ...
  setBlockPosition(this: IToolbar, block: string, position: string) {
    (this.options.positions as Record<string, string>)[block] = position;
    this._showHideButtons();
  ...
  copyDrawControl(
    ...
    const mergedOptions: CustomControlOptions = { ...btn, ...options };
    const control = this.createCustomControl(mergedOptions);
  ...
  createCustomControl(this: IToolbar, options: CustomControlOptions) {
    ...
    const control = this._addButton(
      options.name,
      new L.Control.PMButton(_options) as unknown as IPMButton
    );
    this.changeControlOrder();
    return control;
  },
